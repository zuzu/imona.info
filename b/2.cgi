...
$imonathread = 1099852332;				# iMonaスレッドのスレ番号
###################################################################################################
$ver2cgi = '1.66';	# このスクリプトのバージョン
###################################################################################################
...
$reduceerror = 0;
$cushionpage = 1;	# デフォルトでクッションリンクはONとする
...
# 0x00-0x0F:制御コード
	...
	if($url =~ /^(h?ttp\:\/\/[^\/]+)(\/.+)$/){
		$userver = $1;
		$upath = $2;
		if($userver =~ /^h?ttp\:\/\/[0-9]+$/ || &is2churl($userver)){	# 2chのURL
			...
			if($userver =~ /^h?ttp\:\/\/([0-9]+)$/){	# http://板番号/スレ番号/
				...
				$sboard = $1;	# 板名を取得
				$iboard = url2nbrd($userver . '/' . $1 . '/');	# urlから板番号を取得
				$iboard++;
...
#	}
	...
	if($ver eq '' || ($writemode == 0 && $rawmode == 1) || $writemode == 1 || $shitaraba == 1 || $matibbs == 1){	# リダイレクト
		...
		if($shitaraba == 1){
			...
			$str = "http://same.ula.cc/test/post.so?SRV=$userver&BBS=$ithread&KEY=$sboard";
		} else {
			$str = &boradurl($iboard);
			$str =~ s|(http\://[^/]+/)([^/]+)/|$1test/r.i/$2/$ithread/w|;
		}
        ...
        $userver2 = $userver;
		$userver2 =~ s|http://||g;
        $buf .= " <a href=http://orz.2ch.io/p/-/$userver2/$sboard/$ithread/i>orz</a>";
...
sub ime{
	...
	if(&is2churl($ime)){
		...
		$ime =~ s/#/\$/g;
		$ime =~ s|h?ttp\://|http:\/\/ime\.imona\.info/?|gi;	# ime\.k2y\.info => j\.orz\.hm
		return "$ime";
	}
}
	...
	foreach $str (@data) {
		...
		if($iboard !~ /100/){
			if(length($resbuf[3]) =~ /9/){
				...
				if($resbuf[3] =~ /i$/){	                           #idがiな場合
					$resbuf[3] .= "      Type:iPhone";#idにDocomoを追加
				}
			}
		}
...
sub cnvemoji{
	if($imode == 1){
		...
		$_[0] =~ s/&hearts;/(\xCA\xB0\xC4)/gi;	#ﾊｰﾄ
	}
}
