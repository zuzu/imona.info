...
$usememcached = 0;      #memcachedを利用した板一覧をキャッシュする。(memcachedがインストールされているかmemcachedのサーバー必要ですが、お手軽に高速化が可能です。)
                        #2.cgiの最初の方のrequire Cache::Memcached;以降にてアクセス先のサーバーなどを設定していますので必要に応じて書き換えて下さい。
